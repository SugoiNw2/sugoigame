<?php class BuffTripulacao
{
	/** * @var UserDetails */ private $userDetails;
	/** * @var mywrap_con */ private $connection;
	/** * @var array */ private $buffs_spec;
	/** * @var array */ public $buffs_ativos;
	public function __construct($userDetails, $connection)
	{
		...
		$buffs = $this->connection->run("SELECT * FROM tb_ilha_bonus_ativo WHERE x >= ? AND x <= ? AND y >= ? AND y <= ?", "iiii", array($this->userDetails->tripulacao["x"] - ALCANCE_BONUS_ILHA, $this->userDetails->tripulacao["x"] + ALCANCE_BONUS_ILHA, $this->userDetails->tripulacao["y"] - ALCANCE_BONUS_ILHA, $this->userDetails->tripulacao["y"] + ALCANCE_BONUS_ILHA,))->fetch_all_array();
		foreach ($buffs as $buff) {
			$this->buffs_ativos[] = array_merge($buff, $this->buffs_spec[$buff["buff_id"]]);
		}
	}
	...
	{
		$this->connection->run("DELETE FROM tb_tripulacao_buff WHERE tripulacao_id = ? AND expiracao < ?", "ii", array($this->userDetails->tripulacao["id"], atual_segundo()));
	}
	/** * @param $efeito * @return number|bool */ public function get_efeito($efeito)
	{
		$acumulado = 0;
	...
	{
		$this->connection->run("INSERT INTO tb_tripulacao_buff (tripulacao_id, buff_id, expiracao) VALUE (?,?,?)", "iii", array($this->userDetails->tripulacao["id"], $buff_id, atual_segundo() + $duracao));
	}
	function destroy()
	{
