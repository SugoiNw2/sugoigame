<?php
define('PRECO_GOLD_LUNETA',                             650);
define('PRECO_DOBRAO_LUNETA',                           1560);

define('PRECO_GOLD_PACOTE',                             1500);
define('PRECO_DOBRAO_PACOTE',                           3000);

define('PRECO_GOLD_CONHECIMENTO',                       750);
define('PRECO_DOBRAO_CONHECIMENTO',                     1800);

define('PRECO_GOLD_COUP_DE_BURST',                      450);
define('PRECO_DOBRAO_COUP_DE_BURST',                    1080);

define('PRECO_GOLD_COUP_DE_BURST_INSTANTANEO',          50);
define('PRECO_DOBRAO_COUP_DE_BURST_INSTANTANEO',        100);

define('PRECO_GOLD_TATICAS',                            750);
define('PRECO_DOBRAO_TATICAS',                         1800);

define('PRECO_GOLD_CAMUFLAGEM',                         500);
define('PRECO_DOBRAO_CAMUFLAGEM',                       1200);

define('PRECO_GOLD_RENOMEAR_TRIPULACAO',                1000);
define('PRECO_DOBRAO_RENOMEAR_TRIPULACAO',              2400);

define('PRECO_GOLD_TROCAR_FACCAO',                      1000);
define('PRECO_DOBRAO_TROCAR_FACCAO',                    2400);

define('PRECO_GOLD_FINALIZAR_TRATAMENTO_HOSPITAL',      50);
define('PRECO_DOBRAO_FINALIZAR_TRATAMENTO_HOSPITAL',    100);

define('PRECO_GOLD_ISCA_10',                            20);
define('PRECO_DOBRAO_ISCA_10',                          40);

define('PRECO_GOLD_ISCA_130',                           80);
define('PRECO_DOBRAO_ISCA_130',                         160);

define('PRECO_GOLD_RESET_AKUMA',                        100); // 100
define('PRECO_DOBRAO_RESET_AKUMA',                      240);

define('PRECO_GOLD_REDEFINE_AKUMA',                     50); // 10
define('PRECO_DOBRAO_REDEFINE_AKUMA',                   100);

define('PRECO_GOLD_RESET_ATRIBUTOS',                   20); // 5
define('PRECO_DOBRAO_RESET_ATRIBUTOS',                  50);

define('PRECO_GOLD_RESET_CLASSE',                       20); // 15
define('PRECO_DOBRAO_RESET_CLASSE',                     100);

define('PRECO_GOLD_RESET_DISPOSICAO',                   100);
define('PRECO_DOBRAO_RESET_DISPOSICAO',                 200);

define('PRECO_GOLD_RESET_HAKI',                         100); // 15
define('PRECO_DOBRAO_RESET_HAKI',                       200);

define('PRECO_GOLD_TREINAR_HAKI',                       1);
define('PRECO_DOBRAO_TREINAR_HAKI',                     2);

define('PRECO_GOLD_RESET_NOME_PERSONAGEM',              250);
define('PRECO_DOBRAO_RESET_NOME_PERSONAGEM',            600);

define('PRECO_GOLD_RESET_PROFISSAO',                    100); // 15
define('PRECO_DOBRAO_RESET_PROFISSAO',                  200);

define('PRECO_GOLD_VIAJA_RETORNO',                      200);
define('PRECO_DOBRAO_VIAJA_RETORNO',                    400);

define('PRECO_MODIFICADOR_RECRUTAR_LVL_ALTO',           15);
define('PRECO_MODIFICADOR_DOBRAO_RECRUTAR_LVL_ALTO',    36);

define('PRECO_GOLD_RECUPERAR_OFENSIVA_MESTRE_HAKI',     1);
define('PRECO_DOBRAO_RECUPERAR_OFENSIVA_MESTRE_HAKI',   2);

define('PRECO_GOLD_FERREIRO_VIP',                       50);
define('PRECO_DOBRAO_FERREIRO_VIP',                     100);

define('PRECO_BERRIES_TRANSPORTE_ILHA_RETORNO',         2000000);

define('PRECO_GOLD_TROCAR_PERSONAGEM',                  150);
define('PRECO_DOBRAO_TROCAR_PERSONAGEM',                360);

define('PRECO_GOLD_TROCAR_BANDEIRA',                    250);
define('PRECO_DOBRAO_TROCAR_BANDEIRA',                  500);

define('PRECO_GOLD_RESETAR_INFLUENCIA',                 200);
define('PRECO_DOBRAO_RESETAR_INFLUENCIA',               400);

define('PRECO_GOLD_USAR_FORMACOES',                     100);
define('PRECO_DOBRAO_USAR_FORMACOES',                   200);

define('PRECO_GOLD_CUSTOMIZAR_SKILL',                   50);
define('PRECO_DOBRAO_CUSTOMIZAR_SKILL',                 100);

define('PRECO_GOLD_SELO_EXP',                           25);